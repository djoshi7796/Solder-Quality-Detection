...
Original = pd.read_csv('../results/CURRENT-DATA.csv')
#(A)
...
#Load DATA - Training
Training = pd.read_csv('../results/DATA.csv',header=[0,1])
...
Model_params = pd.read_csv('../results/MODEL-PARAMS.csv')
#(A)
...
#Loss curves
df = pd.read_csv("../results/results.csv",header=None, names=["acc", "loss", "val_acc","val_loss"])
...
#Confusion matrix
metric = pd.read_csv("../results/METRICS.csv")
...
def plot_predictions(fname):
    ...
    l = f.read().splitlines()
    small = l[:4]
    fig_ims = plt.figure(figsize=(10,5))
    for i in range(len(small)//2):
        ax = plt.subplot(240+i+1)
        ax.set_title(os.path.basename(small[i]), fontsize="6")
        ax.imshow(np.uint8(cv2.cvtColor(cv2.imread(small[i]), cv2.COLOR_BGR2RGB)))
        plt.setp(ax.get_xticklabels(), visible=False)
        plt.setp(ax.get_yticklabels(), visible=False)
    for i in range(len(small)//2, len(small)):
        ax = plt.subplot(240+i+len(small)//2+1)
        ax.set_title(os.path.basename(small[i]), fontsize="6")
        ax.imshow(np.uint8(cv2.cvtColor(cv2.imread(small[i]), cv2.COLOR_BGR2RGB)))
        plt.setp(ax.get_xticklabels(), visible=False)
        plt.setp(ax.get_yticklabels(), visible=False)
    ...
    for i in range(len(small)//2):
        ...
        ax.set_title(os.path.basename(full_marked_name), fontsize="6", wrap="True")
        ax.imshow(np.uint8(cv2.cvtColor(cv2.imread(full_marked_name), cv2.COLOR_BGR2RGB)))
        plt.setp(ax.get_xticklabels(), visible=False)
        plt.setp(ax.get_yticklabels(), visible=False)
    for i in range(len(small)//2, len(small)):
        ax = plt.subplot(240+i+len(small)+1)
        splitted = small[i].rsplit('/',2)
        index = splitted[1].index('cropped')
        new_dir = splitted[1][:index]+"checkmark_"+splitted[1][index:]
        index = splitted[2].index('offset')
	...
	<body>
	    ...
	    <h2> RESULTS </h2>
	        ...
	        <h3> Confusion Matrix </h3>'''+conf_mat+'''
                <h3> Accuracy: {0:.3f} &percnt;'''.format(metric['acc'][0]*100)+''' </h3>
                <h3> Precision: {0:.3f} &percnt;'''.format(metric['precision'][0]*100)+''' </h3>
                <h3> Recall: {0:.3f} &percnt;'''.format(metric['recall'][0]*100)+''' </h3>
	        <h2> PREDICTIONS </h2>
                    <h3> True positives </h3>'''+"<img src='data:image/png;base64,{}'>".format(plot_predictions('../results/positives.txt').decode("utf-8"))+'''
                    <h3> False negatives </h3>'''+"<img src='data:image/png;base64,{}'>".format(plot_predictions('../results/negatives.txt').decode("utf-8"))+'''
        </body>
</html>'''
f = open('../results/project_report.html','w')
f.write(html_string)
f.close()
