...
#Change this to local path before running
    ...
    fModel = open('../results/MODEL-PARAMS.csv','w')
    ...
    fDataOrig = open("../results/CURRENT-DATA.csv","w")
    ...
    del Y
    Xdata, Ydata, names = sklearn.utils.shuffle(X, Y_cat, labels)
    del X
    del Y_cat
    del labels
    ...
    fResults = open("../results/results.csv","w")
    ...
    fTP = open("../results/positives.txt","w")
    fFN = open("../results/negatives.txt","w")
    for tp in tp_index:
        if names_val[tp][0]:
